...
const regexScreen = {
  ...
  render(ctx) {
    ...
    wrap.append(
      ...
      card('자주 쓰는 조각',
        // ⚠️ mono( ) 는 글자가 아니라 화면 요소를 돌려준다.
        //    문자열과 + 로 이어 붙이면 [object HTMLElement] 가 찍히므로
        //    반드시 h('span', mono(...), ' 설명') 처럼 나란히 놓아야 한다.
        table(['조각', '뜻', '보기'], [
          [mono('[가-힣]'), '한글 한 글자', h('span', mono('[가-힣]+'), ' → ', b('노트'), '가 좋아요')],
          [mono('[a-zA-Z]'), '영문 한 글자', h('span', mono('[a-zA-Z]+'), ' → ', b('python'), ' 3.12')],
          [mono('[0-9]'), '숫자 한 글자', h('span', mono('[0-9]'), ' → 350 에서 ', b('3'), ', ', b('5'), ', ', b('0'), ' 각각')],
          [mono('[^…]'), '「…이 아닌 것」 (대괄호 안의 ^)', h('span', mono('[^가-힣]'), ' = 한글이 아닌 것')],
          [mono('+'), '앞의 것이 한 번 이상', h('span', mono('[0-9]+'), ' → ', b('350'), ' 을 통째로')],
          [mono('*'), '앞의 것이 없거나 여러 번', h('span', mono('=*'), ' → ', mono('='), ' 이 없어도 걸린다')],
          [mono('?'), '앞의 것이 있거나 없거나', h('span', mono('https?'), ' = http 와 https 둘 다')],
          [mono('.'), '아무 글자 하나', h('span', mono('노.'), ' → ', b('노트'), ', ', b('노래'), ', ', b('노란'))],
          [mono('\\s'), '공백·탭·줄바꿈', h('span', mono('\\s+'), ' → 연속 공백을 통째로')],
          [mono('\\S'), '공백이 아닌 것 (대문자 S)', h('span', mono('http\\S+'), ' → 주소가 끝날 때까지')],
          [mono('{2,}'), '두 번 이상 반복', h('span', mono('ㅋ{2,}'), ' → ', b('ㅋㅋ'), ', ', b('ㅋㅋㅋㅋ'))],
          [mono('|'), '또는', h('span', mono('좋다|나쁘다'), ' → 둘 중 아무거나')],
          [mono('(…)'), '묶어서 기억해 두기', h('span', mono('\\1'), ' 로 기억해 둔 것을 다시 부른다')],
          [mono('\\'), '특수한 뜻을 없앤다', h('span', mono('\\.'), ' → 「아무 글자」가 아니라 진짜 마침표')],
        ], { compact: true }),
        note('info',
          h('b', '대괄호 안의 '), mono('^'), h('b', ' 과 밖의 '), mono('^'), h('b', ' 은 뜻이 다릅니다. '),
          mono('[^가-힣]'), ' 는 「한글이 아닌 글자」이고, ',
          mono('^노트'), ' 는 「', b('줄의 맨 앞'), ' 이 노트로 시작」입니다. 자리로 구별하세요.'),
      ),
